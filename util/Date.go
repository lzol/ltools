...
//将日期字符串转换成日期值
func ConverStrToDate(pattern string,inputDate string)(tm time.Time){
	tm,_ =time.Parse(pattern,inputDate)
	return tm
}

//转换日期字符串格式
func FormatDateString(pattern string,inputDate string,newPattern string)(dateStr string){
	tm := ConverStrToDate(pattern,inputDate)
	return tm.Format(newPattern)
}
